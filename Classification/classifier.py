...
def cross_validate(X,y,normal=True,classifier=svm.svc):
    if normal:
...
class Classifier:
    ...
    def train(self,X_train,y_train,X_test,y_test):
        ...
        clf = self._classifiers.fit(self.X_train,self.y_train)
        accuracy = clf.score(X_test,y_test)
        vote = []
        vote.append([accuracy,self._classifiers])
...
X = preprocess(X)
y = label_encoder(y)
X_train,y_train,X_test,y_test = cross_validate(X,y,normal=True)
